...
def create_data(self):
    ...
    created_at = datetime.fromisoformat(data_now)
...
def delete_data_name_company(self):
    ...
    name = r['name']
    company = r['company']
    d = mongo.db.user.find_one({'name': name, 'company': company})
    data = json_util.dumps(d)
    teste = jsonify({'message': 'Fornecedores -- name= ' + name })
    mongo.db.user.delete_one({'_id': data['_id']})
    response = jsonify({'message': 'Fornecedores -- name= ' + name + ' && company= ' + company + ' Deleted Successfully'})
    response.status_code = 200
    return Response(data, mimetype="application/json", status=302)
